...
import Control.Monad.Trans.State qualified as St
import Data.Binary
...
uniqueListBy ord single distinguish xs0 =
  map fst $
    sortBy (comparing snd) $
      concatMap uniquify $
        groupBy (\x y -> ord (fst x) (fst y) == EQ) $
          sortBy (ord `on` fst) $
            zip xs0 [(0 :: Int) ..]
  where
...
isProgressDisj (DisjG (Disj disj)) =
  all
    ( \f ->
        ( case f of
...
data ProofMethod
  ...
  | -- | A goal that was solved.
    SolveGoal Goal
  | -- | Use inductive strengthening on
...
data DiffProofMethod
  = -- | Proof was not completed
    DiffSorry (Maybe String)
  | -- | No attack was found
    DiffMirrored
  | -- | A potential attack was found
    DiffAttack
  | -- | The backward search is complete (but there are reducible operators in subterms)
    DiffUnfinishable
  | -- | Consider all rules
    DiffRuleEquivalence
  | -- | Do the backward search starting from a rule
    DiffBackwardSearch
  | -- | A step in the backward search starting from a rule
    DiffBackwardSearchStep ProofMethod
  deriving (Eq, Ord, Show, Generic, NFData, Binary)
...
execDiffProofMethod ::
  ...
  DiffProofMethod ->
  DiffSystem ->
  Maybe (M.Map CaseName DiffSystem)
execDiffProofMethod ctxt method sys =
  ...
  where
    ...
    ruleEquivalenceSystem rule =
      L.set dsCurrentRule (Just rule) $
    ...
    backwardSearchSystem s sys' rulename =
      L.set dsSide (Just s) $
...
oracleRanking ::
  (System -> [AnnotatedGoal] -> [AnnotatedGoal]) ->
  Oracle ->
  Bool ->
  ProofContext ->
  System ->
...
internalTacticRanking tactic quitOnEmpty ctxt _sys ags0 = trace logMsg res
  where
    ...
    inp =
      unlines
        ( map
            (\(i, ag) -> show i ++ ": " ++ (concat . lines . render $ pgoal ag))
            (zip [(0 :: Int) ..] ags)
        )
...
injRanking ctxt allowLoopBreakers sys =
  ...
  where
    ...
    isHighPriorityGoal goal =
      (isKnowsFirstNameGoal goal)
        || (isSolveFirstGoal goal)
        || (isChainGoal goal)
        || (isFreshKnowsGoal goal)

    isMedPriorityGoal goaltuple =
    ...
    isLowPriorityGoal goaltuple =
      (isDoubleExpGoal $ fst goaltuple)
...
smartRanking ctxt allowPremiseGLoopBreakers sys =
  ...
  where
    ...
    unmark
      | allowPremiseGLoopBreakers = map unmarkPremiseG
      | otherwise = id
