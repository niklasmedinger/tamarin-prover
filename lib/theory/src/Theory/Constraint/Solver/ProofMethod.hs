...
data ProofMethod =
  ...
  | EnforceBound Int [NodeId]                     -- ^ Enforce that at most n protocol rule instaces are present
...
execProofMethod ctxt method sys =
      case method of
        ...
        EnforceBound b nodeIds
          | length nodeIds > b                 -> execEnforceBound b nodeIds
          | otherwise                          -> Nothing
  where
    ...
    execEnforceBound :: Int -> [NodeId] -> Maybe (M.Map CaseName System)
    execEnforceBound b nodeIds = return $ M.fromListWith (error "case names not unique") cases
      where
        -- We assume that the case names generated by @enforceBound@ are sane.
        cases = removeRedundantCases ctxt [] snd
               . map (second cleanupSystem . fst) . getDisj $ reduc
        reduc  = runReduction solver ctxt sys (avoid sys)
        solver = do name <- enforceBound b nodeIds
                    simplifySystem
                    return name
  ...
  in case rankedGoals of
    ...
    Ranking gs _ -> do
      (m, expl) <-
          ...
          <|> applyBoundedConstraintSolving
          <|> (solveGoalMethod <$> gs)
      case execProofMethod ctxt m sys of
        Just cases -> return (m, (cases, expl))
        Nothing    -> []
  where
    ruleInstances = L.get sNodes sys
    protoRuleInstances = M.toList $ M.filter (\ru -> isProtocolRule ru && not (isFreshRule ru)) ruleInstances
    -- ^ Protocol rule instances without `Fresh' rule instances
    protoRuleInstanceIds = map fst protoRuleInstances
    numProtoRules = length protoRuleInstanceIds
    -- TODO-NM: This computes the wrong bound. It seems to include
    -- rule instances which have already been merged with other rule instances.
    contradiction c                    = (Contradiction (Just c), "")

    applyBoundedConstraintSolving = case L.get pcBound ctxt of
      Just b  | numProtoRules > b -> [(EnforceBound b protoRuleInstanceIds, "More than " ++ show b ++ " protocol rule instances")]
              -- ^ Only apply the rule if there are more than b protocol rules
      _ -> [] -- No bounded solving if the option is not present or the bound is not crossed.
...
prettyProofMethod method = case method of
    ...
    EnforceBound b _     -> keyword_ ("Bound " ++ show b)
    Sorry reason         ->
